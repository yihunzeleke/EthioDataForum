...
font_add("fa-brands", regular = "C:/Fonts/webfonts/fa-brands-400.ttf")
font_add("fa-solid", regular = "C:/Fonts/webfonts/fa-solid-900.ttf")
...
df_pop <- dat$places |>
  ...
  ungroup()
min_max <- function(value, min_range, max_range) {
  scaled_value <- (value - min_range) / (max_range - min_range)
  return(pmax(pmin(scaled_value, 1), 0))
}
...
df_alien <- tibble(
  x = 0,
  y = -5,
  image = image_read("Scripts/ufo.png") |>
...
iom_source <- glue("International Organization for Migration")
caption <- glue("{twitter}{space2}@natabyan{space2}{github}{space2}yihunzeleke/EthioDataForum{space2}{floppy}{space2}source {iom_source}")
...
quote1 <- ggplot() +
  annotate("text",
    x = 0, y = 1, label = str_wrap(quotes[1], 40),
...
quote2 <- ggplot() +
  annotate("text",
    x = 0, y = 1, label = str_wrap(quotes[2], 25),
    family = ft1, colour = txt, size = 16, hjust = 0, fontface = "italic", lineheight = 0.4
  ) +
  xlim(0, 1) +
  ylim(0, 1) +
...
quote3 <- ggplot() +
  annotate("text",
    x = 0, y = 1, label = str_wrap(quotes[3], 25),
    family = ft1, colour = txt, size = 16, hjust = 0, fontface = "italic", lineheight = 0.4
  ) +
  xlim(0, 1) +
  ylim(0, 1) +
...
ggsave(plot = g_final, filename = "Scripts//ufo_allien.png", height = 16, width = 10)
